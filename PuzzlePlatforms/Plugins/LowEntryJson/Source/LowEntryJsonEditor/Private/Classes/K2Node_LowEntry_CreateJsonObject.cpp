...
/////////////////////////////////////////////////////
// FKCHandler_LowEntry_CreateJsonObject

class FKCHandler_LowEntry_CreateJsonObject : public FNodeHandlingFunctor
{
public:
	FKCHandler_LowEntry_CreateJsonObject(FKismetCompilerContext& InCompilerContext) : FNodeHandlingFunctor(InCompilerContext)
	{
	}

	virtual void RegisterNets(FKismetFunctionContext& Context, UEdGraphNode* Node) override
	{
		UK2Node_LowEntry_CreateJsonObject* ArrayNode = CastChecked<UK2Node_LowEntry_CreateJsonObject>(Node);
...
/////////////////////////////////////////////////////
// UK2Node_LowEntry_CreateJsonObject
...
FNodeHandlingFunctor* UK2Node_LowEntry_CreateJsonObject::CreateNodeHandler(FKismetCompilerContext& CompilerContext) const
{
	return new FKCHandler_LowEntry_CreateJsonObject(CompilerContext);
}

FText UK2Node_LowEntry_CreateJsonObject::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	return FText::FromString("Create Json Object");
}

UEdGraphPin* UK2Node_LowEntry_CreateJsonObject::GetArrayOutputPin() const
{
	return FindPin(TEXT("TempArray"));
}

UEdGraphPin* UK2Node_LowEntry_CreateJsonObject::GetObjectOutputPin() const
{
	return FindPin(TEXT("JsonObject"));
}

bool UK2Node_LowEntry_CreateJsonObject::IsNodePure() const
{
	return false;
}

void UK2Node_LowEntry_CreateJsonObject::AllocateDefaultPins()
{
	if(!false)
	{
...
void UK2Node_LowEntry_CreateJsonObject::NotifyPinConnectionListChanged(UEdGraphPin* Pin)
{
...
FText UK2Node_LowEntry_CreateJsonObject::GetTooltipText() const
{
	return FText::FromString("Creates a new Json Object.");
}

void UK2Node_LowEntry_CreateJsonObject::AddInputPin()
{
	FScopedTransaction Transaction(LOCTEXT("AddPinTx", "AddPin"));
	Modify();
...
void UK2Node_LowEntry_CreateJsonObject::RemoveInputPin(UEdGraphPin* Pin)
{
	FScopedTransaction Transaction(LOCTEXT("RemovePinTx", "RemovePin"));
	Modify();
...
void UK2Node_LowEntry_CreateJsonObject::GetContextMenuActions(const FGraphNodeContextMenuBuilder& Context) const
{
	...
	if(!Context.bIsDebugging)
	{
		...
		if(Context.Pin != NULL)
		{
			...
			if((Context.Pin->Direction == EGPD_Input) && (Context.Pin->PinType.PinCategory != UEdGraphSchema_K2::PC_Exec))
			{
				...
				Context.MenuBuilder->AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
					FExecuteAction::CreateUObject(this, &UK2Node_LowEntry_CreateJsonObject::RemoveInputPin, const_cast<UEdGraphPin*>(Context.Pin))
					)
					);
			}
		}
		else
		{
			Context.MenuBuilder->AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
				FExecuteAction::CreateUObject(this, &UK2Node_LowEntry_CreateJsonObject::AddInputPin)
				)
				);
		}
...
void UK2Node_LowEntry_CreateJsonObject::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
...
FText UK2Node_LowEntry_CreateJsonObject::GetMenuCategory() const
{
	return FText::FromString("Low Entry|Json|Object|Create (Callable)");
}

...
/////////////////////////////////////////////////////
// FKCHandler_LowEntry_CreateJsonObjectPure

class FKCHandler_LowEntry_CreateJsonObjectPure : public FNodeHandlingFunctor
{
public:
	FKCHandler_LowEntry_CreateJsonObjectPure(FKismetCompilerContext& InCompilerContext) : FNodeHandlingFunctor(InCompilerContext)
	{
	}

	virtual void RegisterNets(FKismetFunctionContext& Context, UEdGraphNode* Node) override
	{
		UK2Node_LowEntry_CreateJsonObjectPure* ArrayNode = CastChecked<UK2Node_LowEntry_CreateJsonObjectPure>(Node);
	...
	virtual void Compile(FKismetFunctionContext& Context, UEdGraphNode* Node) override
	{
		...
		UFunction* ConvertFunctionPointer = FindField<UFunction>(ConvertFunctionClass, TEXT("Object_Pure_CreateFromObjectEntryArray"));
...
/////////////////////////////////////////////////////
// UK2Node_LowEntry_CreateJsonObjectPure
...
FNodeHandlingFunctor* UK2Node_LowEntry_CreateJsonObjectPure::CreateNodeHandler(FKismetCompilerContext& CompilerContext) const
{
	return new FKCHandler_LowEntry_CreateJsonObjectPure(CompilerContext);
}

FText UK2Node_LowEntry_CreateJsonObjectPure::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	return FText::FromString("Create Json Object");
}

UEdGraphPin* UK2Node_LowEntry_CreateJsonObjectPure::GetArrayOutputPin() const
{
	return FindPin(TEXT("TempArray"));
}

UEdGraphPin* UK2Node_LowEntry_CreateJsonObjectPure::GetObjectOutputPin() const
{
	return FindPin(TEXT("JsonObject"));
}

bool UK2Node_LowEntry_CreateJsonObjectPure::IsNodePure() const
{
	return true;
}

void UK2Node_LowEntry_CreateJsonObjectPure::AllocateDefaultPins()
{
	if(!true)
	{
	...
	UEdGraphPin* ObjectOutputPin = CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Object, TEXT(""), ULowEntryJsonObject::StaticClass(), TEXT("JsonObject"));
	FCreatePinParams ArrayOutputPinParams = FCreatePinParams();
	ArrayOutputPinParams.ContainerType = EPinContainerType::Array;
	UEdGraphPin* ArrayOutputPin = CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Object, TEXT(""), ULowEntryJsonObjectEntry::StaticClass(), TEXT("TempArray"), ArrayOutputPinParams);
	...
	{
		CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object, TEXT(""), ULowEntryJsonObjectEntry::StaticClass(), *FString::Printf(TEXT("[%d]"), i));
	}
}

void UK2Node_LowEntry_CreateJsonObjectPure::NotifyPinConnectionListChanged(UEdGraphPin* Pin)
{
...
FText UK2Node_LowEntry_CreateJsonObjectPure::GetTooltipText() const
{
	return FText::FromString("Creates a new Json Object.");
}

void UK2Node_LowEntry_CreateJsonObjectPure::AddInputPin()
{
	FScopedTransaction Transaction(LOCTEXT("AddPinTx", "AddPin"));
	Modify();
...
void UK2Node_LowEntry_CreateJsonObjectPure::RemoveInputPin(UEdGraphPin* Pin)
{
	FScopedTransaction Transaction(LOCTEXT("RemovePinTx", "RemovePin"));
	Modify();
...
void UK2Node_LowEntry_CreateJsonObjectPure::GetContextMenuActions(const FGraphNodeContextMenuBuilder& Context) const
{
	...
	if(!Context.bIsDebugging)
	{
		...
		if(Context.Pin != NULL)
		{
			...
			if((Context.Pin->Direction == EGPD_Input) && (Context.Pin->PinType.PinCategory != UEdGraphSchema_K2::PC_Exec))
			{
				...
				Context.MenuBuilder->AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
					FExecuteAction::CreateUObject(this, &UK2Node_LowEntry_CreateJsonObjectPure::RemoveInputPin, const_cast<UEdGraphPin*>(Context.Pin))
					)
					);
			}
		}
		else
		{
			Context.MenuBuilder->AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
				FExecuteAction::CreateUObject(this, &UK2Node_LowEntry_CreateJsonObjectPure::AddInputPin)
				)
				);
		}
...
void UK2Node_LowEntry_CreateJsonObjectPure::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
...
FText UK2Node_LowEntry_CreateJsonObjectPure::GetMenuCategory() const
{
	return FText::FromString("Low Entry|Json|Object|Create (Pure)");
}

...
/////////////////////////////////////////////////////
// FKCHandler_LowEntry_CreateJsonArrayPure

class FKCHandler_LowEntry_CreateJsonArrayPure : public FNodeHandlingFunctor
{
public:
	FKCHandler_LowEntry_CreateJsonArrayPure(FKismetCompilerContext& InCompilerContext) : FNodeHandlingFunctor(InCompilerContext)
	{
	}

	virtual void RegisterNets(FKismetFunctionContext& Context, UEdGraphNode* Node) override
	{
		UK2Node_LowEntry_CreateJsonArrayPure* ArrayNode = CastChecked<UK2Node_LowEntry_CreateJsonArrayPure>(Node);
...
/////////////////////////////////////////////////////
// UK2Node_LowEntry_CreateJsonArrayPure
...
FNodeHandlingFunctor* UK2Node_LowEntry_CreateJsonArrayPure::CreateNodeHandler(FKismetCompilerContext& CompilerContext) const
{
	return new FKCHandler_LowEntry_CreateJsonArrayPure(CompilerContext);
}

FText UK2Node_LowEntry_CreateJsonArrayPure::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	return FText::FromString("Create Json Array");
}

UEdGraphPin* UK2Node_LowEntry_CreateJsonArrayPure::GetArrayOutputPin() const
{
	return FindPin(TEXT("TempArray"));
}

UEdGraphPin* UK2Node_LowEntry_CreateJsonArrayPure::GetObjectOutputPin() const
{
	return FindPin(TEXT("JsonArray"));
}

bool UK2Node_LowEntry_CreateJsonArrayPure::IsNodePure() const
{
	return true;
}

void UK2Node_LowEntry_CreateJsonArrayPure::AllocateDefaultPins()
{
	if(!true)
	{
	...
	UEdGraphPin* ObjectOutputPin = CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Object, TEXT(""), ULowEntryJsonArray::StaticClass(), TEXT("JsonArray"));
	FCreatePinParams ArrayOutputPinParams = FCreatePinParams();
	ArrayOutputPinParams.ContainerType = EPinContainerType::Array;
	UEdGraphPin* ArrayOutputPin = CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Object, TEXT(""), ULowEntryJsonValue::StaticClass(), TEXT("TempArray"), ArrayOutputPinParams);
	...
	{
		CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object, TEXT(""), ULowEntryJsonValue::StaticClass(), *FString::Printf(TEXT("[%d]"), i));
	}
}

void UK2Node_LowEntry_CreateJsonArrayPure::NotifyPinConnectionListChanged(UEdGraphPin* Pin)
{
...
FText UK2Node_LowEntry_CreateJsonArrayPure::GetTooltipText() const
{
	return FText::FromString("Creates a new Json Array.");
}

void UK2Node_LowEntry_CreateJsonArrayPure::AddInputPin()
{
	FScopedTransaction Transaction(LOCTEXT("AddPinTx", "AddPin"));
	Modify();
...
void UK2Node_LowEntry_CreateJsonArrayPure::RemoveInputPin(UEdGraphPin* Pin)
{
	FScopedTransaction Transaction(LOCTEXT("RemovePinTx", "RemovePin"));
	Modify();
...
void UK2Node_LowEntry_CreateJsonArrayPure::GetContextMenuActions(const FGraphNodeContextMenuBuilder& Context) const
{
	...
	if(!Context.bIsDebugging)
	{
		...
		if(Context.Pin != NULL)
		{
			...
			if((Context.Pin->Direction == EGPD_Input) && (Context.Pin->PinType.PinCategory != UEdGraphSchema_K2::PC_Exec))
			{
				...
				Context.MenuBuilder->AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
					FExecuteAction::CreateUObject(this, &UK2Node_LowEntry_CreateJsonArrayPure::RemoveInputPin, const_cast<UEdGraphPin*>(Context.Pin))
					)
					);
			}
		}
		else
		{
			Context.MenuBuilder->AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
				FExecuteAction::CreateUObject(this, &UK2Node_LowEntry_CreateJsonArrayPure::AddInputPin)
				)
				);
		}
...
void UK2Node_LowEntry_CreateJsonArrayPure::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
...
FText UK2Node_LowEntry_CreateJsonArrayPure::GetMenuCategory() const
{
	return FText::FromString("Low Entry|Json|Array|Create (Pure)");
}

...
// init >>
	...
	ULowEntryJsonObject* ULowEntryJsonObject::Merge(ULowEntryJsonObject* JsonObject1, ULowEntryJsonObject* JsonObject2)
	{
		...
		{
			for(auto i = JsonObject1->Data->Values.CreateConstIterator(); i; ++i)
			{
				Instance->Data->Values.Add(i->Key, i->Value);
			}
		}
		if(JsonObject2 != nullptr)
		{
			for(auto i = JsonObject2->Data->Values.CreateConstIterator(); i; ++i)
			{
				Instance->Data->Values.Add(i->Key, i->Value);
			}
		}
